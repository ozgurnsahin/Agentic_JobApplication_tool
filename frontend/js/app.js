...
class JobAppAI {
        ...
        contentBody.innerHTML = `
            <div class="card">
                <div class="card-header">
                    ...
                    <p class="card-subtitle">View and manage your optimized CVs for different job opportunities</p>
                </div>
                <div class="card-body">
                    <div style="display: flex; gap: 1rem; margin-bottom: 1rem;">
                        <button class="btn btn-primary" onclick="jobApp.generateCVs()" id="generateCVsBtn">
                            Generate CVs
                        </button>
                        <button class="btn btn-secondary" onclick="jobApp.loadCVsList()">
                            Refresh List
                        </button>
                    </div>
                </div>
    ...
    async loadCVsList() {
        try {
            ...
            if (response && response.cvs && response.cvs.length > 0) {
                const tableHeaders = ['Job Title', 'Company', 'Match Score', 'Created', 'Actions'];
                const tableRows = response.cvs.map(cv => [
                    `<strong>${escapeHtml(cv.job_title || 'N/A')}</strong>`,
                    escapeHtml(cv.company || 'N/A'),
                    cv.match_score ? 
                        `<span class="badge ${this.getScoreBadgeType(cv.match_score)}">${cv.match_score}%</span>` : 
                        '<span class="badge badge-secondary">N/A</span>',
                    formatDate(cv.created_at),
                    `
                        <button class="btn btn-sm btn-primary" onclick="jobApp.downloadCV(${cv.cv_id})">
                            Download
                        </button>
                        <button class="btn btn-sm btn-secondary" onclick="jobApp.viewCVDetails(${cv.cv_id})">
                            Details
                        </button>
                    `
                ]);
                ...
                container.innerHTML = createEmptyState(
                    'No CVs Found', 
                    'No optimized CVs have been generated yet. Click "Generate CVs" to create tailored CVs for your unprocessed jobs.',
                    '<button class="btn btn-primary" onclick="jobApp.generateCVs()">Generate CVs</button>'
                );
            }
        } catch (error) {
            ...
            container.innerHTML = createEmptyState(
                'Failed to Load CVs', 
                'Unable to fetch CVs from the database. Please try again.',
                '<button class="btn btn-primary" onclick="jobApp.loadCVsList()">Retry</button>'
            );
    ...
    async generateCVs() {
        try {
            const generateBtn = document.getElementById('generateCVsBtn');
            if (generateBtn) {
                generateBtn.disabled = true;
                generateBtn.textContent = 'Generating CVs...';
            }
            
            showLoading(true);
            const response = await api.startCVGeneration();
            showLoading(false);
            
            if (response) {
                showToast('CV generation started successfully', 'success');
                
                // Update agent status to show progress
                this.updateAgentStatus();
                
                // Refresh CV list after a delay to show new CVs
                setTimeout(() => {
                    this.loadCVsList();
                }, 3000);
            }
            ...
            showLoading(false);
            showToast('Failed to start CV generation', 'error');
        } finally {
            const generateBtn = document.getElementById('generateCVsBtn');
            if (generateBtn) {
                generateBtn.disabled = false;
                generateBtn.textContent = 'Generate CVs';
            }
        }
    }
    ...
    async downloadCV(cvId) {
        try {
            ...
            if (blob) {
                downloadBlob(blob, `optimized_cv_${cvId}.pdf`);
                showToast('CV downloaded successfully', 'success', 2000);
            }
        } catch (error) {
            showLoading(false);
    ...
    async viewCVDetails(cvId) {
        try {
            const response = await api.getCVs();
            const cv = response.cvs.find(c => c.cv_id === cvId);
            
            if (cv) {
                const content = `
                    <div class="form-group">
                        <label class="form-label">Job Title</label>
                        <p>${escapeHtml(cv.job_title || 'N/A')}</p>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Company</label>
                        <p>${escapeHtml(cv.company || 'N/A')}</p>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Match Score</label>
                        <p>${cv.match_score ? `${cv.match_score}%` : 'N/A'}</p>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Created Date</label>
                        <p>${formatDate(cv.created_at)}</p>
                    </div>
                `;
                
                const footer = `
                    <button class="btn btn-secondary" onclick="hideModal()">Close</button>
                    <button class="btn btn-primary" onclick="jobApp.downloadCV(${cv.cv_id}); hideModal();">Download CV</button>
                `;
                
                showModal('CV Details', content, footer);
            }
        } catch (error) {
            showToast('Failed to load CV details', 'error');
        }
    }
